...
function generateTier4() {
  const tier4Content = `// Tier 4: Real-World Application Scenarios Test Suite
...
void main() {
  ...
  group('Scenario 1: Fresh Graduate 30-Day Job Search Journey (Onboarding to First Job Offer)', () {
    test('1.1 User completes profile setup with career interests and target city', () async {
      await PrefsService.setUserName('Rizky Ramadhan');
      await PrefsService.setUserEmail('rizky.ramadhan@alumni.ac.id');
      await PrefsService.setChecklistDocs(['CV ATS-Friendly', 'Portofolio GitHub', 'Ijazah & Transkrip']);
    ...
    test('1.2 User shares job post from LinkedIn, extracts data, and saves as Tersimpan', () async {
      const shareContent = 'Lowongan Junior Flutter Developer di PT Digital Nusantara. Lokasi Jakarta Selatan. Gaji Rp 8.000.000 - 12.000.000 / bulan. Kirim CV ke hrd@digitalnusantara.id';
      final parsed = await TextParserService.extractFromUrlOrText(shareContent);

      final job = JobApplication(
        id: 'job_scenario1_1',
        companyName: parsed.companyName.isNotEmpty ? parsed.companyName : 'PT Digital Nusantara',
        position: parsed.position.isNotEmpty ? parsed.position : 'Junior Flutter Developer',
        status: 'Tersimpan',
    ...
    test('1.3 User applies to job, updates status to Dikirim, and sets 7-day follow-up reminder', () {
      ...
      final appliedJob = initialJob.copyWith(
        ...
        nextActionNote: 'Kirim email follow-up santun ke HR jika belum ada kabar',
      );
    ...
    test('1.4 Recruiter invites for Interview HR, event is scheduled, and widget projection updates', () {
      final now = DateTime.now();
      ...
      final projection = AndroidHomeWidgetService.buildProjection([scheduledJob], now: now);
      final notifId = NotificationService.notificationIdFor(scheduledJob.id);
    ...
    test('1.5 User receives formal offering, evaluates take-home pay, and marks status Diterima', () {
  ...
  group('Scenario 2: Senior Engineer Multi-Pipeline Parallel Applications Management', () {
    test('2.1 User manages 5 concurrent applications across diverse stages', () {
      final now = DateTime.now();
      final pipeline = [
        E2ETestHelper.createSampleJob(id: 'j1', companyName: 'Tokopedia', position: 'Lead Flutter', status: 'Offering', salaryOffered: 'Rp 42.000.000'),
        E2ETestHelper.createSampleJob(id: 'j2', companyName: 'Traveloka', position: 'Principal Mobile', status: 'Interview User', interviewDate: now.add(const Duration(days: 2))),
        E2ETestHelper.createSampleJob(id: 'j3', companyName: 'Gojek', position: 'Senior Android', status: 'Technical Test', testDate: now.add(const Duration(days: 4))),
        E2ETestHelper.createSampleJob(id: 'j4', companyName: 'Shopee', position: 'Tech Lead', status: 'Interview HR', interviewDate: now.add(const Duration(days: 6))),
        E2ETestHelper.createSampleJob(id: 'j5', companyName: 'OldStartup', position: 'Fullstack Dev', status: 'Dikirim', appliedDate: now.subtract(const Duration(days: 20))),
      ];
    ...
    test('2.2 Ghosted application (20 days inactive) is detected and archived via Bulk Mode', () {
      final now = DateTime.now();
    ...
    test('2.3 User performs multi-token search ("Lead Jakarta") across the pipeline', () {
      final pipeline = [
        E2ETestHelper.createSampleJob(companyName: 'Tokopedia', position: 'Lead Flutter', location: 'Jakarta'),
        E2ETestHelper.createSampleJob(companyName: 'Traveloka', position: 'Principal Mobile', location: 'Tangerang'),
        E2ETestHelper.createSampleJob(companyName: 'Shopee', position: 'Tech Lead', location: 'Jakarta'),
      ];

      final results = JobSearchService.filterJobs(pipeline, query: 'Lead Jakarta');
      expect(results.length, equals(2));
    });

    test('2.4 User logs comprehensive technical interview notes on active stage', () {
      final job = E2ETestHelper.createSampleJob(companyName: 'Traveloka', status: 'Interview User');
      final event = E2ETestHelper.createRecruitmentEvent(
        title: 'System Design & State Management Deep Dive',
        type: 'interview',
        notes: 'Diskusi arsitektur offline-first, Hive storage encryption, dan Riverpod async notifier.',
      );

      final updatedJob = job.copyWith(recruitmentEvents: [...job.recruitmentEvents, event]);
      expect(updatedJob.recruitmentEvents.last.notes, contains('offline-first'));
    });

    test('2.5 Pro subscription verification allows advanced export and metrics', () async {
  ...
  group('Scenario 3: Offline-First Heavy Transit Usage (KRL / MRT Commute Flow)', () {
    test('3.1 Cold launch in airplane mode operates smoothly without network calls', () {
    ...
    test('3.2 User transitions status and logs notes completely offline in local Hive storage', () {
      final initialJob = E2ETestHelper.createSampleJob(id: 'j_offline', status: 'Tersimpan');
    ...
    test('3.3 Inline error & retry debouncing prevents flickering during intermittent 4G connection', () {
      var syncAttempts = 0;
      var isSyncing = false;
    ...
    test('3.4 Android Home Widget renders offline cache without network dependencies', () {
    ...
    test('3.5 App returns to online state without losing locally updated records', () {
  ...
  group('Scenario 4: Annual Job Search Archival & Secure Phone Migration', () {
    test('4.1 Annual stats calculation summaries 50 applications over 12 months', () {
    ...
    test('4.2 User generates AES-256 encrypted backup archive before device reset', () async {
    ...
    test('4.3 Simulated fresh install restores database with 100% data integrity', () async {
      ...
      final restored = await BackupService.restoreFromBytes(bytes, password: backupPass);
    ...
    test('4.4 Notification alarms and home widget projections are rebuilt on restored device', () {
      final now = DateTime.now();
      ...
      final projection = AndroidHomeWidgetService.buildProjection([futureInterview], now: now);
      final notifId = NotificationService.notificationIdFor(futureInterview.id);

      expect(projection.hasContent, isTrue);
      expect(notifId, greaterThan(0));
    });

    test('4.5 Search query history and user preferences restore cleanly', () async {
  ...
  group('Scenario 5: Overcoming Rejection & Resilient Career Transition Journey', () {
    test('5.1 User marks dream job application as Ditolak after final interview', () {
      final job = E2ETestHelper.createSampleJob(
      ...
      final rejectedJob = job.copyWith(
        status: 'Ditolak',
        outcomeReason: 'Ditolak di tahap final interview (kandidat internal dipilih)',
        clearNextAction: true,
        closedAt: DateTime.now(),
      );
    ...
    test('5.2 Rejected status cleans up pending reminder alerts without orphan alerts', () {
      final now = DateTime.now();
      ...
      final projection = AndroidHomeWidgetService.buildProjection([rejectedJob], now: now);
    ...
    test('5.4 User creates new application for competing company with upgraded salary expectation', () {
      final newJob = JobApplication(
        ...
        jobDescription: 'Leading a team of 8 Flutter engineers in payment gateway vertical.',
        createdAt: DateTime.now(),
        updatedAt: DateTime.now(),
      );
    ...
    test('5.5 Resilient application progresses rapidly to Offering stage with celebrate mascot', () {
...
`;
  ...
  fs.writeFileSync('test/e2e/tier4_real_world_scenarios_test.dart', tier4Content, 'utf8');
  console.log('tier4_real_world_scenarios_test.dart generated successfully (5 Real-World Scenarios, 25 tests).');
}

module.exports = { generateTier4 };
