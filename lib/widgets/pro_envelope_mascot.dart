...
class _ProKingPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final goldFill = Paint()..color = const Color(0xFFF59E0B)..style = PaintingStyle.fill;
    final blushPaint = Paint()..color = const Color(0xFFFF8A80).withValues(alpha: 0.75)..style = PaintingStyle.fill;
    ...
    void drawPlus(Offset center, double s, Color color, double phase) {
      ...
      canvas.drawLine(Offset(center.dx - dynamicSize * scale, center.dy), Offset(center.dx + dynamicSize * scale, center.dy), pPaint);
      canvas.drawLine(Offset(center.dx, center.dy - dynamicSize * scale), Offset(center.dx, center.dy + dynamicSize * scale), pPaint);
    }

    // Top-Left Plus
    drawPlus(Offset((28 + math.sin(t) * 3) * scale, (32 + math.cos(t) * 3) * scale), 9, const Color(0xFFF59E0B), 0);
    drawPlus(Offset((52 - math.cos(t) * 3) * scale, (18 + math.sin(t) * 3) * scale), 5, const Color(0xFF5C44E4), 1.5);

    // Top-Right Plus
    drawPlus(Offset((214 + math.cos(t) * 4) * scale, (30 + math.sin(t) * 4) * scale), 10, const Color(0xFFF59E0B), 3.0);
    drawPlus(Offset((190 + math.sin(t) * 2) * scale, (16 - math.cos(t) * 2) * scale), 6, const Color(0xFF1E8E3E), 4.5);

    // Bottom Sparkles
    drawPlus(Offset(22 * scale, (132 + math.sin(t) * 3) * scale), 7, const Color(0xFF5C44E4), 2.0);
    drawPlus(Offset(218 * scale, (126 - math.cos(t) * 3) * scale), 8, const Color(0xFFF59E0B), 5.0);

    // ── 2. LEGS & SHOES ──
    final leftLeg = Path()..moveTo(85 * scale, 125 * scale)..lineTo(75 * scale, 155 * scale);
    canvas.drawPath(leftLeg, strokePaint);
    final leftShoe = Path()..moveTo(60 * scale, 150 * scale)..lineTo(90 * scale, 150 * scale)..lineTo(85 * scale, 168 * scale)..lineTo(52 * scale, 168 * scale)..close();
    canvas.drawPath(leftShoe, bodyPaint);
    canvas.drawPath(leftShoe, strokePaint);
    canvas.drawLine(Offset(54 * scale, 163 * scale), Offset(83 * scale, 163 * scale), strokePaint);

    final rightLeg = Path()..moveTo(155 * scale, 125 * scale)..lineTo(165 * scale, 155 * scale);
    canvas.drawPath(rightLeg, strokePaint);
    final rightShoe = Path()..moveTo(150 * scale, 150 * scale)..lineTo(180 * scale, 150 * scale)..lineTo(188 * scale, 168 * scale)..lineTo(155 * scale, 168 * scale)..close();
    canvas.drawPath(rightShoe, bodyPaint);
    canvas.drawPath(rightShoe, strokePaint);
    canvas.drawLine(Offset(157 * scale, 163 * scale), Offset(186 * scale, 163 * scale), strokePaint);
    ...
    // Fold lines
    final flapTop = Path()..moveTo(57 * scale, 50 * scale)..lineTo(120 * scale, 92 * scale)..lineTo(183 * scale, 50 * scale);
    canvas.drawPath(flapTop, strokePaint);
    final flapBottom = Path()..moveTo(57 * scale, 128 * scale)..lineTo(105 * scale, 90 * scale);
    canvas.drawPath(flapBottom, strokePaint);
    final flapBottomRight = Path()..moveTo(183 * scale, 128 * scale)..lineTo(135 * scale, 90 * scale);
    ...
    // Joyful Face
    final leftEye = Path()..moveTo(92 * scale, 76 * scale)..quadraticBezierTo(98 * scale, 68 * scale, 104 * scale, 76 * scale);
    canvas.drawPath(leftEye, strokePaint);
    final rightEye = Path()..moveTo(136 * scale, 76 * scale)..quadraticBezierTo(142 * scale, 68 * scale, 148 * scale, 76 * scale);
    canvas.drawPath(rightEye, strokePaint);

    final smile = Path()..moveTo(112 * scale, 82 * scale)..quadraticBezierTo(120 * scale, 92 * scale, 128 * scale, 82 * scale);
    canvas.drawPath(smile, strokePaint);

    canvas.drawOval(Rect.fromLTWH(88 * scale, 80 * scale, 12 * scale, 6 * scale), blushPaint);
    canvas.drawOval(Rect.fromLTWH(140 * scale, 80 * scale, 12 * scale, 6 * scale), blushPaint);

    // Left Arm (Raised victory gesture)
    final leftArm = Path()..moveTo(55 * scale, 85 * scale)..cubicTo(32 * scale, 75 * scale, 25 * scale, 55 * scale, 35 * scale, 42 * scale);
    ...
    final rightArm = Path()..moveTo(185 * scale, 85 * scale)..cubicTo(208 * scale, 75 * scale, 215 * scale, 55 * scale, 205 * scale, 42 * scale);
